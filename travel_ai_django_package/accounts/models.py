...
class UserAIMemory(models.Model):
    """로그인 사용자용 AI 메모리(선호/제약/톤 등). 게스트는 저장하지 않음."""
    user = models.OneToOneField(
    ...
    memories = models.JSONField("메모리", default=list, blank=True)
    updated_at = models.DateTimeField("수정일", auto_now=True)

    class Meta:
        verbose_name = "AI 메모리"
        verbose_name_plural = "AI 메모리"

    def __str__(self):
        return f"AI Memory - {self.user.username}"


class UserChatSession(models.Model):
    """AI 가이드 채팅 세션 (plan/day 단위)."""
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
    ...
    day_number = models.PositiveSmallIntegerField("일차", default=1)
    created_at = models.DateTimeField("생성일", auto_now_add=True)
    updated_at = models.DateTimeField("수정일", auto_now=True)

    class Meta:
        verbose_name = "AI 채팅 세션"
        verbose_name_plural = "AI 채팅 세션"
        ordering = ["-updated_at"]

    def __str__(self):
        plan = f"plan:{self.travel_plan_id}" if self.travel_plan_id else "plan:none"
        return f"{self.user.username} {plan} day:{self.day_number}"
...
class UserChatMessage(models.Model):
    ...
    role = models.CharField("역할", max_length=20, choices=ROLE_CHOICES)
    content = models.TextField("내용")
    sources = models.JSONField("근거", default=list, blank=True)
    actions = models.JSONField("액션", default=list, blank=True)
    used_llm = models.BooleanField("LLM 사용", default=False)
    created_at = models.DateTimeField("생성일", auto_now_add=True)

    class Meta:
        verbose_name = "AI 채팅 메시지"
        verbose_name_plural = "AI 채팅 메시지"
        ordering = ["created_at"]
...
class UserBehaviorEvent(models.Model):
    """개인화 학습을 위한 행동 로그 (동선 최적화/삭제/순서 선호 등)."""
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
    ...
    day_number = models.PositiveSmallIntegerField("일차", default=1)
    event_type = models.CharField("이벤트 타입", max_length=50)
    payload = models.JSONField("페이로드", default=dict, blank=True)
    created_at = models.DateTimeField("생성일", auto_now_add=True)

    class Meta:
        verbose_name = "행동 이벤트"
        verbose_name_plural = "행동 이벤트"
        ordering = ["-created_at"]

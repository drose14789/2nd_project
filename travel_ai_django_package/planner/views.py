...
def _save_assistant_reply(chat_session, reply, sources=None, actions=None, used_llm=False):
    """로그인 사용자일 때만 assistant 메시지를 저장."""
    try:
        if not chat_session:
            return
...
from accounts.models import UserAIMemory, UserChatSession, UserChatMessage, UserBehaviorEvent
...
from services.rag_engine import search as rag_search
from services.llm_client import generate_explanation_openai, is_llm_configured

logger = logging.getLogger(__name__)


# ------------------------------------------------------------
# AI Copilot memory (file-based, no DB migration)
# ------------------------------------------------------------
def _ai_mem_dir() -> str:
    base_dir = getattr(settings, "BASE_DIR", os.getcwd())
    d = os.path.join(base_dir, "storage", "ai_memory")
    os.makedirs(d, exist_ok=True)
    return d

def _ai_mem_path(plan_id: int) -> str:
    return os.path.join(_ai_mem_dir(), f"plan_{plan_id}.json")

def _load_ai_mem(plan_id: int) -> list:
    p = _ai_mem_path(plan_id)
    if not os.path.exists(p):
        return []
    try:
        with open(p, "r", encoding="utf-8") as f:
            obj = json.load(f) or {}
        mem = obj.get("memories") or []
        if isinstance(mem, list):
            return [str(x) for x in mem if str(x).strip()]
    except Exception:
        pass
    return []

def _save_ai_mem(plan_id: int, memories: list) -> None:
    p = _ai_mem_path(plan_id)
    try:
        payload = {
            "plan_id": plan_id,
...
def _load_user_ai_mem(user):
    """로그인 사용자 AI 메모리 로드 (DB)."""
    try:
        obj, _ = UserAIMemory.objects.get_or_create(user=user)
        return obj, (obj.memories or [])
    except Exception:
        return None, []


def _save_user_ai_mem(obj, memories):
    try:
        if obj is None:
            return
        obj.memories = (memories or [])[-50:]
        obj.save(update_fields=["memories", "updated_at"])
    except Exception:
        pass


def _log_user_event(user, travel_plan, day_number, event_type, payload=None):
    try:
...
def _memory_signal_profile(memories: list) -> dict:
    """Parse short memory lines into soft preference signals."""
    txt = " ".join([str(x) for x in (memories or []) if str(x).strip()])
    txt_l = txt.lower()
    prof = {
        'type_weights': {},
        'feature_weights': {},
    ...
    # keyword heuristics (Korean + English)
    def has(*ks):
        return any(k in txt for k in ks) or any(k in txt_l for k in [k.lower() for k in ks])

    if has("미슐", "michelin"):
        prof['michelin_boost'] += 0.8
        prof['type_weights']['Restaurant'] = prof['type_weights'].get('Restaurant', 0.0) + 0.5
    if has("맛집", "식당", "레스토랑", "foodie"):
        prof['type_weights']['Restaurant'] = prof['type_weights'].get('Restaurant', 0.0) + 0.35
    if has("카페", "커피", "베이커리", "cafe"):
        prof['type_weights']['Cafe'] = prof['type_weights'].get('Cafe', 0.0) + 0.45
    if has("랜드마크", "명소", "전망", "타워", "궁", "palace", "landmark"):
        prof['landmark_boost'] += 0.8
        prof['type_weights']['Attraction'] = prof['type_weights'].get('Attraction', 0.0) + 0.2
    if has("자연", "힐링", "공원", "해변", "산", "nature", "park", "beach", "mountain"):
        prof['feature_weights']['자연/힐링'] = prof['feature_weights'].get('자연/힐링', 0.0) + 0.6
    if has("쇼핑", "시장", "아울렛", "백화점", "shopping", "mall", "outlet"):
        prof['feature_weights']['쇼핑'] = prof['feature_weights'].get('쇼핑', 0.0) + 0.6
    if has("여유", "휴식", "빡세", "빡센", "느긋", "relaxed"):
        prof['type_weights']['Cafe'] = prof['type_weights'].get('Cafe', 0.0) + 0.25

    # clamp
    prof['landmark_boost'] = min(1.0, prof['landmark_boost'])
    prof['michelin_boost'] = min(1.0, prof['michelin_boost'])
    return prof


def _behavior_signal_profile(user, travel_plan=None, day_number=None) -> dict:
    """Derive soft signals from logged behavior events."""
    prof = {
        'type_weights': {},
        'feature_weights': {},
        'landmark_boost': 0.0,
    ...
    if not user or not getattr(user, 'is_authenticated', False):
        return prof
    try:
        qs = UserBehaviorEvent.objects.filter(user=user).order_by('-created_at')
        if travel_plan:
            qs = qs.filter(travel_plan=travel_plan)
        if day_number is not None:
            qs = qs.filter(day_number=int(day_number))
        events = list(qs[:200])
        if not events:
            return prof

        rest_first = sum(1 for e in events if e.event_type == "reorder_pref" and (e.payload or {}).get("pref") == "restaurant_first")
        cafe_first = sum(1 for e in events if e.event_type == "reorder_pref" and (e.payload or {}).get("pref") == "cafe_first")

        if rest_first >= 2:
            prof['type_weights']['Restaurant'] = prof['type_weights'].get('Restaurant', 0.0) + min(0.9, rest_first * 0.2)
            prof['michelin_boost'] = min(1.0, prof['michelin_boost'] + 0.3)
        if cafe_first >= 2:
            prof['type_weights']['Cafe'] = prof['type_weights'].get('Cafe', 0.0) + min(0.9, cafe_first * 0.2)

        # If user often removes attractions, gently reduce attraction weight
        rm = [e for e in events if e.event_type == "remove_poi"]
        if len(rm) >= 4:
            prof['type_weights']['Attraction'] = prof['type_weights'].get('Attraction', 0.0) - 0.2
...
def _merge_profiles(*profiles) -> dict:
    out = {'type_weights': {}, 'feature_weights': {}, 'landmark_boost': 0.0, 'michelin_boost': 0.0}
    for p in profiles:
        if not p:
            continue
        out['landmark_boost'] += float(p.get('landmark_boost') or 0.0)
        out['michelin_boost'] += float(p.get('michelin_boost') or 0.0)
        for k,v in (p.get('type_weights') or {}).items():
            out['type_weights'][k] = out['type_weights'].get(k, 0.0) + float(v or 0.0)
        for k,v in (p.get('feature_weights') or {}).items():
            out['feature_weights'][k] = out['feature_weights'].get(k, 0.0) + float(v or 0.0)

    # clamp and trim
    out['landmark_boost'] = min(1.0, max(0.0, out['landmark_boost']))
    out['michelin_boost'] = min(1.0, max(0.0, out['michelin_boost']))
    for d in [out['type_weights'], out['feature_weights']]:
        for k in list(d.keys()):
            d[k] = max(-1.0, min(2.0, float(d[k] or 0.0)))
    return out


def _personalization_profile(user, pref=None, travel_plan=None, day_number=None) -> dict:
    """Unified learn profile: history + DB memory + behavior."""
    base = _history_profile(user)
    memories = []
    if user and getattr(user, 'is_authenticated', False):
        try:
            mem_obj, memories = _load_user_ai_mem(user)
        except Exception:
            memories = []
    mem_prof = _memory_signal_profile(memories[-20:])
    beh_prof = _behavior_signal_profile(user, travel_plan=travel_plan, day_number=day_number)

    # TravelPreference sliders already used directly in scoring; here we only add a tiny bias.
    pref_prof = {'type_weights': {}, 'feature_weights': {}, 'landmark_boost': 0.0, 'michelin_boost': 0.0}
    try:
        style = (_pref_attr(pref, 'travel_style', '') or '').strip()
        if style == 'foodie':
            pref_prof['type_weights']['Restaurant'] = 0.35
            pref_prof['michelin_boost'] = 0.2
        elif style == 'cultural':
            pref_prof['landmark_boost'] = 0.25
            pref_prof['type_weights']['Attraction'] = 0.2
        elif style == 'relaxed':
            pref_prof['type_weights']['Cafe'] = 0.25
        elif style == 'nature':
            pref_prof['feature_weights']['자연/힐링'] = 0.25
    except Exception:
        pass

    return _merge_profiles(base, mem_prof, beh_prof, pref_prof)


def _user_profile_summary(user, pref=None) -> str:
    """Short profile text to inject into LLM prompt."""
    if not user or not getattr(user, 'is_authenticated', False):
        return ""
    lines = []
    try:
        tp = getattr(user, "travel_preference", None)
        if tp:
            lines.append(f"- 여행스타일: {getattr(tp, 'travel_style', 'mixed')}")
            lines.append(f"- 랜드마크 선호도(1~10): {getattr(tp, 'landmark_preference', 7)}")
            lines.append(f"- 미식 선호도(1~10): {getattr(tp, 'food_preference', 5)} / 미슐랭 선호: {bool(getattr(tp, 'michelin_preference', True))}")
            lines.append(f"- 하루 방문 장소 수 선호: {getattr(tp, 'daily_poi_count', 4)}")
            if bool(getattr(tp, 'needs_breaks', True)):
                lines.append(f"- 중간 휴식/카페 포함 선호")
            if bool(getattr(tp, 'avoid_crowds', False)):
                lines.append(f"- 혼잡 회피 선호")
    except Exception:
        pass
    try:
        mem_obj, mem = _load_user_ai_mem(user)
        if mem:
            for m in mem[-5:]:
                lines.append(f"- 기억된 요청: {str(m)}")
    except Exception:
        pass
    if not lines:
        return ""
    return "사용자 개인화 프로필(로그인):\n" + "\n".join(lines) + "\n"




def _personalization_reason_text(pref=None, learn=None) -> str:
    """사용자 개인화 적용 근거를 한 줄로 생성(로그인 사용자용)."""
    bits = []
    try:
        style = (_pref_attr(pref, "travel_style", "") or "").strip()
        if style:
            ...
            bits.append(f"스타일: {style_map.get(style, style)}")
    except Exception:
        pass
    try:
        type_w = (learn or {}).get("type_weights") or {}
        top = sorted(type_w.items(), key=lambda x: float(x[1] or 1.0), reverse=True)[:1]
        if top:
            t, w = top[0]
            if float(w or 1.0) > 1.05:
                kmap = {"Restaurant":"맛집", "Attraction":"명소", "Cafe":"카페"}
                bits.append(f"최근 행동 기반 선호: {kmap.get(t, t)}")
    except Exception:
        pass
    try:
        if float((learn or {}).get("michelin_boost", 0.0) or 0.0) >= 0.4:
            bits.append("미슐랭 선호 반영")
        if float((learn or {}).get("landmark_boost", 0.0) or 0.0) >= 0.4:
            bits.append("랜드마크 선호 반영")
    except Exception:
        pass
    if not bits:
        return ""
    return "개인화 적용: " + ", ".join(bits[:3])


def _personalized_rag_keywords(pref=None, learn=None, message: str = "") -> str:
    """RAG 검색어를 유저 스타일에 맞게 확장."""
    style = (_pref_attr(pref, 'travel_style', '') or 'mixed').strip()
    food = float(_pref_attr(pref, 'food_preference', 5) or 5)
    lm = float(_pref_attr(pref, 'landmark_preference', 7) or 7)
    avoid_crowds = bool(_pref_attr(pref, 'avoid_crowds', False))
    needs_breaks = bool(_pref_attr(pref, 'needs_breaks', True))
    mic_pref = bool(_pref_attr(pref, 'michelin_preference', True))

    lm_boost = float((learn or {}).get('landmark_boost', 0.0) or 0.0)
    mi_boost = float((learn or {}).get('michelin_boost', 0.0) or 0.0)
    type_w = (learn or {}).get('type_weights', {}) or {}

    kws = []
    # universal travel info
    kws += ["hours", "ticket", "reservation", "how to go", "metro", "bus", "entrance", "best time", "crowd", "tips", "transport"]

    # user bias
    foodie = (style == 'foodie') or (food >= 8) or (type_w.get('Restaurant', 1.0) > 1.15) or (mi_boost >= 0.4)
    landmarky = (style in ['cultural','active','nature']) or (lm >= 8) or (type_w.get('Attraction', 1.0) > 1.15) or (lm_boost >= 0.4)

    if foodie:
        kws += ["Michelin", "signature dish", "menu", "booking", "waiting", "reservation", "price", "must-try"]
        if mic_pref:
            kws += ["Michelin star", "Michelin guide"]
    if landmarky:
        kws += ["history", "architecture", "viewpoint", "photo spot", "route", "walking", "palace", "museum"]
    if include_korean := True:
        # Korean variants help for KR corpus
        if foodie:
            kws += ["미슐랭", "대표메뉴", "예약", "웨이팅", "맛집", "가격"]
        if landmarky:
            kws += ["역사", "관람시간", "입장권", "동선", "포토스팟", "전망"]
        if avoid_crowds:
            kws += ["혼잡", "붐비는 시간", "피크시간", "피하는 법"]
        if needs_breaks:
            kws += ["휴식", "카페", "중간 쉼"]

    # If user asked something specific, let message dominate; we just add bias tail.
    # de-dup
    out=[]
    seen=set()
    for k in kws:
...
def generate_plan(request):
    ...
    try:
        with transaction.atomic():
            ...
            learn = _personalization_profile(request.user, pref=pref, travel_plan=travel_plan, day_number=None) if request.user.is_authenticated else {
                'type_weights': {},
                'feature_weights': {},
                'landmark_boost': 0.0,
                'michelin_boost': 0.0,
...
def day_explain_api(request, plan_id, day_number):
    ...
    try:
        ...
        # --- Optional: client-side current order (to avoid "설명" mismatch right after reorder) ---
        client_order = data.get("poi_order") or data.get("poiOrder") or []
        if isinstance(client_order, list):
            norm = []
            for x in client_order:
                try:
                    norm.append(int(x))
                except Exception:
                    continue
        ...
        # If the client has already reordered the DOM (and possibly is still saving),
        # sync the DB order here so the next "일정 설명" reflects what the user sees.
        if client_order:
            try:
                db_ids = [int(p.id) for p in pois]
                if client_order != db_ids and set(client_order) == set(db_ids):
                    by_id = {int(p.id): p for p in pois}
                    for idx, pid in enumerate(client_order):
                        if pid in by_id:
                            by_id[pid].order = idx
                            by_id[pid].save(update_fields=["order"])
                    pois = list(day_plan.pois.all().order_by("order"))
            except Exception:
                # If anything goes wrong, just continue with DB order
                pass
...
@require_POST
def copilot_chat_api(request, plan_id, day_number):
    """AI Copilot chat endpoint for the day page.

    This is a lightweight orchestrator:
    - Uses RAG (services.rag_engine) for grounded answers.
    - Optionally uses LLM if OPENAI_API_KEY is configured.
    - Returns simple UI actions so the front-end can modify the route:
        - optimize : run client-side nearest-neighbor optimization
        - remove_poi: remove a POI by id
    - Stores very small "memory" (preferences) per plan (file-based).
    """
    try:
        travel_plan = get_object_or_404(TravelPlan, id=plan_id)
        day_plan = get_object_or_404(DayPlan, travel_plan=travel_plan, day_number=day_number)
        ...
        # --- DB: 로그인 사용자 채팅 세션/메시지 저장 ---
        chat_session = None
        if request.user.is_authenticated:
            try:
                chat_session, _ = UserChatSession.objects.get_or_create(
                    user=request.user,
                    travel_plan=travel_plan,
                    day_number=int(day_number),
                )
        ...
        # --- Load / update simple memory (preferences) ---
        if request.user.is_authenticated:
            mem_obj, memories = _load_user_ai_mem(request.user)
        else:
            mem_obj = None
            memories = _load_ai_mem(plan_id)
        # crude preference capture: store short user constraints to reuse later
        pref_patterns = [r"앞으로", r"다음부터", r"선호", r"원해", r"해주세요", r"해줘"]
        if any(re.search(p, message) for p in pref_patterns):
            m = re.sub(r"\s+", " ", message).strip()
            if m and m not in memories:
                memories = (memories + [m])[-20:]
                if request.user.is_authenticated:
                    _save_user_ai_mem(mem_obj, memories)
                else:
                    _save_ai_mem(plan_id, memories)

        # --- Action detection (keep conservative for stability) ---
        actions = []

        # 1) Optimize request
        if re.search(r"(최적화|가까운|동선|이동\s*편|가까운\s*순서|동선\s*좋게)", message):
            actions.append({"type": "optimize"})
            if request.user.is_authenticated:
                _log_user_event(request.user, travel_plan, day_number, "optimize_route", {"city": day_plan.to_city})
            reply = "좋아요! 이동이 편한 순서로 동선을 정리해볼게요. (동선 최적화 실행)"
            _save_assistant_reply(chat_session, reply, sources=[{'title': h.get('title'), 'url': h.get('url'), 'source': h.get('source'), 'snippet': h.get('snippet')} for h in hits], actions=actions, used_llm=used_llm)
        ...
        # 1.5) Reorder preference (e.g., "맛집 먼저", "카페 먼저")
        # We keep this conservative and return UI actions so the front-end can apply the order instantly.
        if re.search(r"((맛집|식당|레스토랑)\s*(을|를)?\s*(먼저|우선)|((먼저|우선)\s*(맛집|식당|레스토랑)))", message) or re.search(r"(restaurant).*(first|earlier)", message, re.I):
            actions.append({"type": "reorder_pref", "pref": "restaurant_first"})
            if request.user.is_authenticated:
                _log_user_event(request.user, travel_plan, day_number, "reorder_pref", {"pref": "restaurant_first", "city": day_plan.to_city})
            reply = "좋아요! '맛집 먼저' 흐름(숙소→맛집→관광→카페→관광→숙소)에 가깝게 동선을 재배치할게요."
            return JsonResponse({
                "ok": True,
                "reply": reply,
                "sources": [],
        ...
        if re.search(r"((카페)\s*(를|을)?\s*(먼저|우선)|((먼저|우선)\s*카페))", message):
            actions.append({"type": "reorder_pref", "pref": "cafe_first"})
            if request.user.is_authenticated:
                _log_user_event(request.user, travel_plan, day_number, "reorder_pref", {"pref": "cafe_first", "city": day_plan.to_city})
            reply = "좋아요! '카페 먼저' 흐름에 가깝게 동선을 재배치할게요."
            return JsonResponse({
                "ok": True,
                "reply": reply,
                "sources": [],
        ...
        # 2) Remove request (match by name, or remove last cafe/restaurant)
        if re.search(r"(삭제|빼|제거)", message):
            ...
            # heuristic: "카페" -> remove last cafe-like
            if target is None and re.search(r"카페", msg_norm):
                for p in reversed(pois):
                    t = (getattr(p, "poi_type", "") or getattr(p, "category", "") or "").lower()
                    if "cafe" in t or "카페" in t or "coffee" in t:
                        target = p
                        break
            # heuristic: "맛집/식당" -> remove last restaurant-like
            if target is None and re.search(r"(맛집|식당|레스토랑)", msg_norm):
                for p in reversed(pois):
                    t = (getattr(p, "poi_type", "") or getattr(p, "category", "") or "").lower()
                    if "restaurant" in t or "food" in t or "맛집" in t or "레스토랑" in t:
                        target = p
                        break

            if target is not None:
                actions.append({"type": "remove_poi", "poi_id": int(target.id)})
                if request.user.is_authenticated:
                    _log_user_event(request.user, travel_plan, day_number, "remove_poi", {"poi_id": int(target.id), "poi_name": (target.name or "").strip(), "city": day_plan.to_city})
                reply = f"좋아요! '{(target.name or '').strip()}'를 일정에서 제거할게요."
            else:
                reply = "어떤 장소를 제거할지 찾지 못했어요. (예: 'OOO 빼줘')처럼 이름을 같이 말해줘도 좋아요."
        ...
        # 2.5) Schedule explanation request ("일정 설명해줘", "루트 설명", etc.)
        # - This is different from general RAG Q&A: it should summarize the *current itinerary*.
        if re.search(r"(일정|루트|동선).*(설명|요약|가이드)", message) or re.search(r"(설명|요약).*(일정|루트|동선)", message) or re.search(r"(schedule|itinerary).*(explain|summary)|((explain|summary).*(schedule|itinerary))", message, re.I):
            try:
                sig = _ai_explain_signature(day_plan, pois)
                cached = _load_ai_explain(plan_id, day_number)
                if cached and cached.get("signature") == sig and cached.get("text"):
                    return JsonResponse({
                        "ok": True,
                        "reply": cached.get("text", ""),
                        "sources": cached.get("sources", []) or [],
                        "actions": [],
                        "memories": memories,
                        "used_llm": bool(cached.get("used_llm", False)),
                        "cached": True,
                    })
                ...
                logger.exception("schedule explain failed")
                return JsonResponse({"ok": False, "error": str(e)}, status=500)


        # --- Grounded answer (RAG / optional LLM) ---
        poi_dicts = []
        for p in pois:
            poi_dicts.append({
                "id": p.id,
        ...
        if request.user.is_authenticated:
            try:
                pref = getattr(request.user, "travel_preference", None)
                # travel_plan/day_number 기반 행동로그 반영
                learn = _personalization_profile(request.user, pref=pref, travel_plan=travel_plan, day_number=day_number)
            except Exception:
                pref = None
                learn = None

        city = (day_ctx.get("city") or "").strip()
        # include memories as soft constraints
        mem_text = " \n".join([f"- {m}" for m in memories[-8:]])
        personal_kw = _personalized_rag_keywords(pref=pref, learn=learn, message=message)
        query = (
            f"{city} {message} 여행 팁 교통 동선 혼잡 예약 safety transit metro train reservation crowd {personal_kw} "
            + " ".join([x.get("name", "") for x in (day_ctx.get("points") or [])[:8]])
        )
        hits = rag_search(query=query, city=city, top_k=5)
        ...
        used_llm = False
        reply = ""
        if is_llm_configured() and hits:
            used_llm = True
            profile_summary = ""
            personal_reason = ""
            if request.user.is_authenticated:
                try:
                    profile_summary = _user_profile_summary(request.user, pref=pref, learn=learn)
                    personal_reason = _personalization_reason_text(pref=pref, learn=learn)
                except Exception:
                    profile_summary = ""
                    personal_reason = ""
            sys = (
                "너는 10년 경력의 현지 전문 여행 가이드이자 일정 컨설턴트야.\n"
                "말투는 친절하지만 단정한 전문가 톤(한국어)으로, 불필요한 과장/추측은 금지해.\n"
                "반드시 제공된 일정 정보와 Sources에 근거해서만 말하고, Sources에 없는 사실은 만들어내지 마.\n"
                "답변은 핵심만 4~8문장으로, 필요하면 짧은 bullet 2~4개를 추가해.\n"
                "근거가 필요한 문장 끝에는 [1] [2] 처럼 번호 인용을 붙여.\n"
                "사용자 요청이 동선/순서 변경이면, 실행 가능한 제안(예: 맛집 먼저/카페 먼저/시나리오 최적화)을 한 줄로 요약하고 이유를 덧붙여.\n"
            )
            user = (
                f"도시: {day_ctx.get('country','')} / {city}\n"
                f"오늘 루트: "
                + " → ".join([p.get('name','') for p in day_ctx.get('points',[]) if p.get('name')])
                + "\n\n"
                + (f"기억된 요청:\n{mem_text}\n\n" if mem_text else "")
                + f"사용자 메시지: {message}\n\n"
                + "Sources:\n" + (sources_block if sources_block else "(no sources)")
            )
            reply = generate_explanation_openai(system=sys, user=user, max_tokens=320, temperature=0.2) or ""

        if not reply:
            # deterministic fallback: stitch 1~2 snippets into a short answer
            parts = []
            for i, h in enumerate(hits[:2], start=1):
                ...
                sn = re.split(r"[.!?。！？]\s*", sn)[0]
                if sn:
                    parts.append(f"{sn} [{i}]")
            if parts:
                reply = "\n".join([f"- {p}" for p in parts])
            else:
                reply = """현재 보유한 근거 문서에서는 질문에 바로 연결되는 정보를 찾지 못했어요. 👇
원하시면 **우선순위(교통/혼잡/예약/안전/예산)** 중 어떤 걸 가장 중요하게 볼지 알려주면, 그 기준으로 더 정확히 안내해드릴게요."""

        if personal_reason:
            # 채팅 답변 하단에만 개인화 근거를 1줄로 표시(일정 UI에는 영향 없음)
            reply = (reply.strip() + "\n\n—\n" + personal_reason).strip()

        return JsonResponse({
            "ok": True,
            "reply": reply.strip(),
            "personalization": personal_reason,
            "sources": hits,
            "actions": actions,
            "memories": memories,
            "used_llm": bool(used_llm),
        ...
        logger.exception("copilot_chat_api failed")
        return JsonResponse({"ok": False, "error": str(e)}, status=500)
...
def export_pdf(request, plan_id):
    ...
    def _trip_overview(dp) -> bool:
        ...
        pts = _points_for_day(dp)
                ...
                # --- Right column: route map + tips (kept strictly inside the card) ---
        # NOTE: 이전 버그에서 map_y/map_h 조합으로 카드(top)를 넘어가 제목 영역을 침범했음.
        #       여기서는 카드 내부(top margin 12)로 고정하고, 그 아래 여유 공간에 Tips를 배치함.
        map_w, map_h = 520, 260
        map_x = x + 340
        map_y = y + h - map_h - 12  # keep map top inside the card

        map_img = _make_route_map(pts, size=(map_w, map_h))
        cnv.drawImage(ImageReader(map_img), map_x, map_y, width=map_w, height=map_h, mask="auto")

        # --- Tips box under the route map (LLM+RAG) ---
        # 요구사항: "지도 아래 남는 공간"에 자연스럽게 들어가고, 배경 노란색 제거 + 오버플로우 방지
        tips_payload = _get_or_make_day_tips(dp)
        if tips_payload and (tips_payload.get("bullets") or []):
            try:
                tbx = map_x
                tbw = map_w
                gap = 10
                tips_bottom_min = y + 18  # keep a little padding from card bottom
                max_tbh = max(0, int(map_y - tips_bottom_min - gap))

                # Build wrapped lines first, then size the box to fit (up to available height).
                line_h = 10.5
                title_h = 16
                pad_top = 12
                pad_bot = 10
                ...
                raw_lines = []
                for b in (tips_payload.get("bullets", []) or [])[:3]:
                    b = (b or "").strip()
                    if not b:
                        continue
                    # strip leading bullets and trailing citations like "[1]"
                    b = re.sub(r"^\s*[•\-*]\s*", "", b).strip()
                    b = re.sub(r"\s*\[[0-9]+\]\s*$", "", b).strip()
                    if not b:
                        continue

                    wrapped = _wrap_to_lines(b, tbw - 28, FONT, font_size, max_lines=999)
                    if not wrapped:
                        continue
                    # add bullet prefix and indent following lines
                    wrapped[0] = "• " + wrapped[0]
                    for i in range(1, len(wrapped)):
                        wrapped[i] = "  " + wrapped[i]
                    raw_lines.extend(wrapped)

                if raw_lines and max_tbh >= 44:
                    max_lines_fit = int((max_tbh - title_h - pad_top - pad_bot) / line_h)
                    if max_lines_fit > 0:
                        lines = raw_lines[:max_lines_fit]
                        tbh = int(min(max_tbh, title_h + pad_top + pad_bot + len(lines) * line_h))
                        tby = int(map_y - tbh - gap)

                        # box (no yellow; subtle border)
                        cnv.setFillColor(Color(1, 1, 1))
                        cnv.setStrokeColor(Color(0.90, 0.90, 0.90))
                        cnv.roundRect(tbx, tby, tbw, tbh, 14, stroke=1, fill=1)

                        cnv.setFont(FONT_B, 10)
                        cnv.setFillColor(C_TITLE)
                        cnv.drawString(tbx + 14, tby + tbh - 16, "Tips")

                        cnv.setFont(FONT, font_size)
                        cnv.setFillColor(C_TITLE)

                        yy = tby + tbh - 30
                        for ln in lines:
                            cnv.drawString(tbx + 14, yy, ln)
                            yy -= line_h
            except Exception:
                pass
...
def _personalized_day_quota(pref=None, learn=None, include_cafe=True, daily_poi_count=4):
    """일자별 POI 구성 비율을 개인화(선호+학습)로 결정.
    Returns dict: {'Attraction':a, 'Restaurant':r, 'Cafe':c}
    """
    try:
    ...
    # base: (A=2,R=1,C=1) for n>=4
    a = 2 if n >= 4 else 1
    r = 1 if n >= 2 else 0
    c = 1 if (include_cafe and n >= 4) else (1 if (include_cafe and n >= 3) else 0)
    ...
    foodie = (style == 'foodie') or (food >= 8) or (type_w.get('Restaurant', 1.0) > 1.15) or (mi_boost >= 0.4)
    landmarky = (style in ['cultural', 'active', 'nature']) or (lm >= 8) or (type_w.get('Attraction', 1.0) > 1.15) or (lm_boost >= 0.4)
    cafe_like = include_cafe and (needs_breaks or type_w.get('Cafe', 1.0) > 1.12)

    # Adjust quotas
    if n >= 4:
        if foodie and not landmarky:
            a = 1
            r = 2
            c = 1 if include_cafe else 0
        elif landmarky and not foodie:
            a = 3
            r = 1
            c = 0 if not cafe_like else 1
        elif foodie and landmarky:
            a = 2
            r = 2 if n >= 5 else 1
            c = 1 if include_cafe else 0
    else:
        ...
        if n == 3:
            if foodie:
                a, r, c = 1, 2, (0 if not include_cafe else 0)
            elif landmarky:
                a, r, c = 2, 1, 0
            else:
                a, r, c = 2, 1, 0
    ...
    # fill missing
    while total < n:
        if include_cafe and cafe_like and c < 2:
            c += 1
        elif landmarky and a < 4:
            a += 1
        elif r < 2:
            r += 1
        else:
    ...
    return {'Attraction': a, 'Restaurant': r, 'Cafe': c}
...
def _select_default_pois(candidates, pref=None, include_cafe=True, learn=None, daily_poi_count=4, exclude_place_ids=None, exclude_names=None, anchor_lat=None, anchor_lon=None, travel_order=True):
    ...
    quota = _personalized_day_quota(pref=pref, learn=learn, include_cafe=include_cafe, daily_poi_count=daily_poi_count)
    # 타입별 분류
    ...
    if sorted_restaurants:
        ...
        # --- 개인화: 맛집 quota(최대 2) 반영 ---
        try:
            want_r = int((quota or {}).get('Restaurant', 1) or 1)
        except Exception:
            want_r = 1
        want_r = max(1, min(2, want_r))
        cur_r = len([x for x in selected if x.get('type') == 'Restaurant'])
        if want_r > cur_r:
            # 두 번째 맛집은 첫 맛집/관광지 클러스터 근처에서 고름
            try:
                base_lat, base_lon = None, None
                if chosen_rest and chosen_rest.get('latlng'):
                    base_lat, base_lon = chosen_rest.get('latlng')
                elif tgt_lat is not None and tgt_lon is not None:
                    base_lat, base_lon = tgt_lat, tgt_lon
                cand2 = None
                if base_lat is not None and base_lon is not None:
                    best = None
                    best_cost = None
                    for r2 in sorted_restaurants[:min(35, len(sorted_restaurants))]:
                        if r2 == chosen_rest or r2 in selected:
                            continue
                        lat, lon = (r2.get('latlng') or (None, None))
                        if lat is None or lon is None:
                            continue
                        d = _haversine_km(base_lat, base_lon, lat, lon)
                        rt = float(r2.get('rating') or 0.0)
                        rv = int(r2.get('reviews') or r2.get('google_review_count') or 0)
                        val = rt * 2.0 + math.log1p(max(rv, 0)) * 0.2
                        if r2.get('michelin_grade'):
                            val += 2.0
                        cost = d * 2.6 - val
                        if best_cost is None or cost < best_cost:
                            best_cost = cost
                    ...
                    cand2 = best
                if not cand2:
                    cand2 = next((x for x in sorted_restaurants if x not in selected), None)
                if cand2:
    ...
    # 카페 (개인화 quota 반영)
    try:
        want_c = int((quota or {}).get('Cafe', 0) or 0)
    except Exception:
        want_c = 0
    want_c = max(0, min(2, want_c))

    # 카페 1개
    if include_cafe and cafes and want_c > 0:
        ...
        if want_c > 1:
            try:
                base_lat, base_lon = None, None
                if chosen_cafe and chosen_cafe.get('latlng'):
                    base_lat, base_lon = chosen_cafe.get('latlng')
                elif chosen_rest and chosen_rest.get('latlng'):
                    base_lat, base_lon = chosen_rest.get('latlng')
                cand2 = None
                if base_lat is not None and base_lon is not None:
                    best = None
                    best_cost = None
                    for c2 in cafes_sorted[:min(35, len(cafes_sorted))]:
                        if c2 == chosen_cafe or c2 in selected:
                            continue
                        lat, lon = (c2.get('latlng') or (None, None))
                        if lat is None or lon is None:
                            continue
                        d = _haversine_km(base_lat, base_lon, lat, lon)
                        rt = float(c2.get('rating') or 0.0)
                        rv = int(c2.get('reviews') or c2.get('google_review_count') or 0)
                        val = rt * 2.0 + math.log1p(max(rv, 0)) * 0.15
                        cost = d * 2.4 - val
                        if best_cost is None or cost < best_cost:
                            best_cost = cost
                    ...
                    cand2 = best
                if not cand2:
                    cand2 = next((x for x in cafes_sorted if x not in selected), None)
                if cand2:
                    selected.append(cand2)
            except Exception:
                pass
    ...
    # 개인화 quota 기반으로 이미 구성되었으므로, 초과하면 뒤에서부터 제거(카페→관광지→맛집 순)
    if len(selected) > n:
        def _type(x): return (x or {}).get('type') or ''
        # prefer keep at least 1 attraction if possible, and 1 restaurant if n>=2
        while len(selected) > n:
            # remove a cafe if any
            idx = next((i for i in range(len(selected)-1, -1, -1) if _type(selected[i])=='Cafe'), None)
            if idx is not None:
                selected.pop(idx)
                continue
            # remove an attraction beyond 1
            attr_idx = [i for i,x in enumerate(selected) if _type(x)=='Attraction']
            if len(attr_idx) > 1:
                selected.pop(attr_idx[-1])
                continue
            # remove a restaurant beyond 1
            rest_idx = [i for i,x in enumerate(selected) if _type(x)=='Restaurant']
            if len(rest_idx) > 1:
                selected.pop(rest_idx[-1])
                continue
            selected.pop()  # fallback
...
def personalization_dashboard(request):
    """회원 개인화 대시보드: 선호/학습/최근 행동/최근 채팅/개인화 근거를 한 화면에."""
    user = request.user
    pref = getattr(user, "travel_preference", None)

    learn_all = _personalization_profile(user, pref=pref, travel_plan=None, day_number=None)

    try:
        events = user.behavior_events.all()[:30]
    except Exception:
        events = []
    try:
        sessions = user.chat_sessions.all()[:10]
    except Exception:
        sessions = []
    try:
        msgs = UserChatMessage.objects.filter(session__user=user, role="assistant").select_related("session")[:15]
    except Exception:
        msgs = []

    kpi = {
        "type_weights": (learn_all or {}).get("type_weights") or {},
        "michelin_boost": float((learn_all or {}).get("michelin_boost", 0.0) or 0.0),
        "landmark_boost": float((learn_all or {}).get("landmark_boost", 0.0) or 0.0),
        "top_events": (learn_all or {}).get("top_events") or [],
    }

    profile_summary = _user_profile_summary(user, pref=pref, learn=learn_all)
    return render(
        request,
        "planner/personalization_dashboard.html",
        {
            "pref": pref,
            "learn": learn_all,
            "kpi": kpi,
